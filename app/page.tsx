...
export default function App() {
  ...
  return (
    <div style={{padding:'20px', maxWidth:'900px', margin:'0 auto'}}>
      <h1 style={{textAlign:'center', fontSize:'36px', fontFamily:'serif'}}>Become Your Own Lover</h1>
      <div style={{display:'flex', gap:'8px', marginBottom:'20px', flexWrap:'wrap', justifyContent:'center', borderBottom:'1px solid #eee', paddingBottom:'10px'}}>
        {Object.keys(data).map(cat => (
          <button key={cat} onClick={()=>{setActiveTab(cat); setDay(1)}}
            style={{padding:'10px 16px', background:'none', border:'none', borderBottom: activeTab===cat? '2px solid #f97316' : '2px solid transparent', cursor:'pointer', fontSize:'16px'}}>
            {cat}
          </button>
        ))}
      </div>
      ...
      <div style={{padding:'20px'}}>
        ...
        {locked? (
          <div style={{textAlign:'center', padding:'40px'}}>
            <img src={currentImage} alt="prompt" style={{width:'100%', height:'300px', objectFit:'cover', borderRadius:'8px', marginBottom:'16px', opacity:0.3}} />
            <p style={{fontSize:'18px', marginBottom:'16px'}}>🔒 Unlock Day {day} and all other categories</p>
            <button onClick={handleCheckout} style={{padding:'14px 28px', background:'#06b6d4', color:'white', border:'none', borderRadius:'8px', fontSize:'16px', cursor:'pointer', fontWeight:'bold'}}>
